/**
 * Public Request Repository
 *
 * Handles caching and retrieval of public endpoint requests/responses.
 * Maintains retention limit: keeps only the latest 5 versions per endpoint configuration.
 *
 * Public endpoints are version-specific and do NOT require user authentication.
 *
 * @module repositories/public_request_repository
 */
...
/**
 * Find existing public request by matching criteria
 * Matches: endpoint_path, method, query_params (based on config or full match),
 * request_body (based on match_body config), app_platform, app_version,
 * app_environment, app_language, is_secure=0
 *
 ...
 */
function findExistingPublicRequest(
  db,
  endpointPath,
  method,
...
/**
 * Save a public endpoint request/response
 * Automatically cleans up old versions after insertion to maintain 5-version limit
 *
 * Note: query_params are normalized (sorted keys, lowercase) before storage for consistent matching
 *
 * @param {number} userId - User ID (if available, may be null)
 ...
 * @param {string} mobileHeaders.version - App version
 ...
 * @param {string} endpointType - 'transmit', 'secure', or 'public' (default: 'public')
 * @returns {Object} Created request object
 * @throws {Error} If save fails
 */
async function savePublicRequest(
  userId = null,
  endpoint,
  method,
  queryParams = null,
  ...
  try {
    ...
    // For matching, we'll use normalized version in findExistingPublicRequest
    ...
    // Pass original queryParams for partial matching support
    // Pass body for body field matching when recording config has match_body
    const existingRequest = findExistingPublicRequest(
      db,
      endpointPath,
      method,
      normalizedQueryParamsJson,
    ...
    if (existingRequest) {
      ...
      logger.info(`Public request updated: ${endpointName} (ID: ${requestId})`);
    } else {
      ...
      logger.info(`Public request inserted: ${endpointName} (ID: ${requestId})`);
    }
    ...
    logger.error("Failed to save public request:", error);
    throw new Error(`Failed to save public request: ${error.message}`);
  }
}
...
function updateResponseBody(requestId, newResponseBody) {
  try {
    ...
    logger.info(`Updated response body for request ${requestId}`);
...
function getRequestById(requestId) {
  try {
    ...
    const stmt = db.prepare(`
      ...
      WHERE r.id = ? AND r.endpoint_type = 'public'
    `);
